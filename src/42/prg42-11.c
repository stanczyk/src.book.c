...
int main (void)
{
	const unsigned char * msg = (unsigned char *) u8"źdźbła zbóź";
	// const char * msg = u8"\U0001f61d \xF0\x9F\x87\xB5 \xF0\x9F\x87\xB1";
	setlocale(LC_ALL, "en_US.utf8");
	// setlocale (LC_ALL, "pl_PL.UTF-8");
	mbstate_t stan;
	memset (&stan, 0, sizeof stan);
	size_t dl = 1 + mbsrtowcs(NULL, (const char **__restrict) &msg, 0, &stan);
	wchar_t wyn[dl];
	mbsrtowcs(&wyn[0], (const char **__restrict) &msg, dl, &stan);
